...
class UrbanSound8K(td.Dataset):
    def __init__(
        ...
        **_,
        ...
        self.label_to_class_idx = {
            lb: idx for idx, lb in self.class_idx_to_label.items()
        }

    @staticmethod
    def _load_worker(
        fn: str, path_to_file: str, sample_rate: int, mono: bool = False
    ...
    def load_data(self):
        ...
        if self.random_split_seed is not None:
            ...
            skf = skms.StratifiedKFold(
                n_splits=10, shuffle=True, random_state=self.random_split_seed
            )
        ...
        with mp.Pool(processes=num_processes) as pool:
            ...
            for fn, sample_rate, wav in pool.starmap(
                ...
                iterable=[
                    (fn, path, sr, mono) for fn, (path, sr, mono) in self.data.items()
                ],
