...
with torch.no_grad():
    ...
    for fold in range(1, 11):
        fold_df = sorted_df[sorted_df["fold"] == fold]
        y_preds, y_labels = [], []
        for file_path, target in tqdm(
            zip(fold_df["slice_file_name"], fold_df["classID"]), total=len(fold_df)
        ):
            audio_path = pre_path + file_path
            one_hot_target = (
                torch.zeros(len(classes))
                .scatter_(0, torch.tensor(target), 1)
                .reshape(1, -1)
            )
            audio, _ = librosa.load(audio_path, sr=32000, mono=True)
            audio = torch.tensor(audio).unsqueeze(0).to(device)
        ...
        y_labels, y_preds = (
            np.concatenate(y_labels, axis=0),
