...
def load_metadata(dataset, csv_file):
    ...
    for i, item in enumerate(csv_list):
        ...
        if caption_field is not None:
            item_captions = [
                _sentence_process(item[cap_ind], add_specials=False)
                for cap_ind in caption_field
            ]
        else:
...
def pack_dataset_to_hdf5(dataset):
    ...
    for split in splits:
        ...
        with h5py.File(hdf5_path + "{}.h5".format(split), "w") as hf:
            hf.create_dataset(
                "audio_name", shape=(audio_nums,), dtype=h5py.special_dtype(vlen=str)
            )
            hf.create_dataset("audio_length", shape=(audio_nums,), dtype=np.uint32)
            hf.create_dataset(
                "waveform", shape=(audio_nums, max_audio_length), dtype=np.float32
            )

            if split == "train" and dataset == "AudioCaps":
                hf.create_dataset(
                    "caption", shape=(audio_nums,), dtype=h5py.special_dtype(vlen=str)
                )
            else:
                hf.create_dataset(
                    "caption", shape=(audio_nums, 5), dtype=h5py.special_dtype(vlen=str)
                )
            ...
            for i in tqdm(range(audio_nums)):
                ...
                audio, _ = librosa.load(
                    audio_dir + audio_name, sr=sampling_rate, mono=True
                )
        ...
        logger.info(
            f"Packed {split} set to {hdf5_path} using {time.time() - start_time} s."
        )
