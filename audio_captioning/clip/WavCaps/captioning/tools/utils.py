...
def decode_output(
    predicted_output, ref_captions, file_names, log_output_dir, epoch, beam_size=1
):
    if beam_size != 1:
        logging = logger.add(
            str(log_output_dir)
            + "/beam_captions_{}ep_{}bsize.txt".format(epoch, beam_size),
            format="{message}",
            level="INFO",
        ...
        logging = logger.add(
            str(log_output_dir) + "/captions_{}ep.txt".format(epoch),
            format="{message}",
            level="INFO",
