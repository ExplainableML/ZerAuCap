...
def main():
    ...
    parser.add_argument("-l", "--lr", default=1e-04, type=float, help="Learning rate.")
    ...
    main_logger.info(
        f"Size of training set: {len(train_loader.dataset)}, size of batches: {len(train_loader)}"
    )
    main_logger.info(
        f"Size of validation set: {len(val_loader.dataset)}, size of batches: {len(val_loader)}"
    )
    main_logger.info(
        f"Size of test set: {len(test_loader.dataset)}, size of batches: {len(test_loader)}"
    )
