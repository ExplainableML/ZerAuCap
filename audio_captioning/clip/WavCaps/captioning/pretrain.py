...
def train(model, dataloader, optimizer, scheduler, device, epoch):
    ...
    for batch_id, (audio, text, audio_names, _) in tqdm(
    ...
    with torch.no_grad():
        ...
        if beam_size == 3 and (epoch % 5) == 0:
            for metric, values in metrics.items():
                val_logger.info(
                    f'beam search (size 3): {metric:<7s}: {values["score"]:7.4f}'
                )
...
def main():
    ...
    parser.add_argument("-l", "--lr", default=1e-05, type=float, help="Learning rate.")
