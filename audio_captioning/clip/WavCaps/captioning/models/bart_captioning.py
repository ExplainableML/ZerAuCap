...
class BartCaptionModel(nn.Module):
    def __init__(self, config):
        ...
        encoder_config = AudioEncoderConfig(
            **config["audio_encoder_args"], audio_args=config["audio_args"]
        )
        ...
        self.enc_to_dec_proj = nn.Linear(
            encoder_config.hidden_size, self.decoder.config.hidden_size
        )
    ...
    def forward_decoder(self, text, encoder_outputs):
        encoder_outputs = self.decoder.model.encoder(
            input_ids=None, inputs_embeds=encoder_outputs, return_dict=True
        )["last_hidden_state"]
        ...
        loss = self.loss_fct(
            lm_logits.view(-1, self.tokenizer.vocab_size), decoder_targets.view(-1)
        )
    ...
    def generate(
        ...
        input_ids = (
            torch.zeros((encoder_outputs["last_hidden_state"].size(0), 1))
            .long()
            .to(self.device)
        )
        ...
        decoder_attention_mask = (
            torch.ones((encoder_outputs["last_hidden_state"].size(0), 1))
            .long()
            .to(self.device)
        )
